...
def _resolve_auto_device():
    # CUDA is NVIDIA-only, so torch.cuda.is_available() is always False on a
    # Mac — 'auto' used to silently fall back to plain CPU on every Apple
    # Silicon machine even though PyTorch's MPS backend (Metal, the GPU)
    # is available and meaningfully faster for Whisper. Check MPS before
    # giving up and landing on CPU.
    if torch.cuda.is_available():
        return 'cuda'
    if torch.backends.mps.is_available():
        return 'mps'
...
def ensure_model(request):
    ...
    if requested_device == 'auto':
        requested_device = _resolve_auto_device()
    ...
    if requested_device == 'mps' and not torch.backends.mps.is_available():
        raise RuntimeError(
            'MPS was requested but this Python environment/machine does not support it'
        )
